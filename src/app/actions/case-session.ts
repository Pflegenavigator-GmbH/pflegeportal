...
export async function validateAndStoreSession(caseCode: string): Promise<SessionStatus> {
    try {
        ...
        if (error || !currentCase) {
            return { success: false, isUnlocked: false, isExpired: false, billingStatus: 'pending', caseCode: null };
        }
        ...
        const isUnlocked = (currentCase.billing_status === 'paid' || currentCase.billing_status === 'free') && !isExpired;
        ...
        return { success: false, isUnlocked: false, isExpired: false, billingStatus: 'failed', caseCode: null };
    }
}