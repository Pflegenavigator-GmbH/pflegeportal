...
export function TagebuchForm({
  ...
  const [helfer, setHelfer] = useState<PflegeHelfer>('Hauptpflegeperson');
  const [schmerzen, setSchmerzen] = useState<number>(5);
  const [schlaf, setSchlaf] = useState<SchlafQualitaet>('Gut');

  // Besonderheiten Tags
  const [sturz, setSturz] = useState(false);
  const [arzttermin, setArzttermin] = useState(false);
  const [krankenhaus, setKrankenhaus] = useState(false);
  const [bettlaegerig, setBettlaegerig] = useState(false);
  const [medikamentenKontrolle, setMedikamentenKontrolle] = useState(false);
  ...
  useEffect(() => {
    if (entryToEdit) {
      const d = entryToEdit.data;
      setDatum(d.date.split('T')[0]);
      setContent(d.content);
      setHelfer(d.helfer || 'Hauptpflegeperson');
      setSchmerzen(d.schmerzen ?? 5);
      setSchlaf(d.schlaf || 'Gut');
      setSturz(!!d.sturz);
      setArzttermin(!!d.arzttermin);
      setKrankenhaus(!!d.krankenhaus);
      setBettlaegerig(!!d.bettlaegerig);
      setMedikamentenKontrolle(!!d.medikamentenKontrolle);
    } else {
      setDatum(new Date().toISOString().split('T')[0]);
      setContent('');
      setHelfer('Hauptpflegeperson');
      setSchmerzen(5);
      setSchlaf('Gut');
      setSturz(false);
      setArzttermin(false);
      setKrankenhaus(false);
      setBettlaegerig(false);
      setMedikamentenKontrolle(false);
    }
  }, [entryToEdit]);

  const speichern = async () => {
    if (!content.trim()) return toast.error('Bitte beschreiben Sie, was nicht alleine gemacht werden konnte.');
    setIsSaving(true);
  ...
  return (
      <Card className="bg-white border border-gray-200 shadow-xl text-slate-900">
        ...
        <CardContent className="p-5 space-y-4 text-sm">
          ...
          {/* Datum mit nativem Browser-Dropdown/Kalender */}
          <div>
            ...
            <Input
                ...
                onClick={(e) => (e.target as any).showPicker?.()} // Erzwingt das Öffnen des Dropdowns bei Klick
          ...
          {/* Wer hat geholfen */}
          <div>
            <label className="block font-bold text-xs uppercase text-gray-500 mb-1">Wer hat heute geholfen?</label>
            <select
                value={helfer}
          ...
          {/* Schlaf-Struktur */}
          <div>
            <label className="block font-bold text-xs uppercase text-gray-500 mb-2">Wie war der Schlaf nachts?</label>
          ...
          <div className="pt-2 border-t border-gray-100 space-y-2">
            <label className="block font-bold text-xs uppercase text-gray-500 mb-1">Besonderheiten / Ereignisse</label>
            ...
            <div className="grid grid-cols-2 gap-2 text-slate-900">
              <label className="flex items-center gap-2 p-2 rounded border border-gray-200 cursor-pointer hover:bg-gray-50">
                <input type="checkbox" checked={sturz} onChange={(e) => setSturz(e.target.checked)} className="rounded text-[#1a4480]" />
                <span>Sturz / Unfall</span>
              </label>
              <label className="flex items-center gap-2 p-2 rounded border border-gray-200 cursor-pointer hover:bg-gray-50">
                <input type="checkbox" checked={arzttermin} onChange={(e) => setArzttermin(e.target.checked)} className="rounded text-[#1a4480]" />
                <span>Arzttermin</span>
              </label>
              <label className="flex items-center gap-2 p-2 rounded border border-gray-200 cursor-pointer hover:bg-gray-50">
                <input type="checkbox" checked={krankenhaus} onChange={(e) => setKrankenhaus(e.target.checked)} className="rounded text-[#1a4480]" />
                <span>Krankenhaus</span>
              </label>
              <label className="flex items-center gap-2 p-2 rounded border border-gray-200 cursor-pointer hover:bg-gray-50">
                <input type="checkbox" checked={bettlaegerig} onChange={(e) => setBettlaegerig(e.target.checked)} className="rounded text-[#1a4480]" />
            ...
            <label className="flex items-center gap-2 p-2 mt-1 rounded bg-blue-50 border border-blue-100 cursor-pointer text-slate-900">
              ...
              />
              <span className="text-xs text-slate-700 font-medium">Medikamentenkontrolle durch Pfledienst erfolgt?</span>
            </label>
          </div>

          {/* Action Buttons (Mindesthöhe 56px gemäß Block 18.9) */}
