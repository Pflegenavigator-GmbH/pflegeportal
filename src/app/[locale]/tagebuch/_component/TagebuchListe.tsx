...
const MVP_PRODUCTS = [{ id: 'beta_special', name: 'Beta-Special (12 Monate)', price_cents: 2900 }];
  ...
  return (
    <div className="space-y-3">
      ...
      {Object.entries(grouped).map(([month, items]) => (
        <div
          key={month}
          className="border border-white/10 rounded-xl overflow-hidden bg-slate-950/20 text-xs"
        >
          <button
            onClick={() =>
              setOpenMonths((p) =>
                p.includes(month) ? p.filter((m) => m !== month) : [...p, month]
              )
            }
              ...
              {items.map(({ key, entry }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-300 mt-2 font-sans line-clamp-1 break-all">
                      {entry.content}
                    </p>
                    <div className="mt-2 flex flex-wrap gap-1 items-center">
                      <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-blue-500/10 border border-blue-500/20 text-blue-400">
                        S: {entry.schmerzen}/10
                      </span>
                      {entry.sturz && (
