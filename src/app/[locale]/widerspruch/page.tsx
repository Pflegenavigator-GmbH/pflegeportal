...
import {
    FileText, Clock, Info, ArrowLeft, Eye, Save, Trash2, Bell, Mail
} from 'lucide-react';

// Hooks, Modals & Utils Imports
...
import {
    WiderspruchDaten, WiderspruchFrist, WiderspruchTyp, berechneFrist, generiereWiderspruchBrief, formatiereFristInfo
...
export default function WiderspruchPage(props: PageProps) {
    ...
    const isDatumGueltig = bescheidDatum !== '' && !isNaN(new Date(bescheidDatum).getTime()) && bescheidDatum <= heuteISO;
    ...
    const handleKalkulation = async () => {
        ...
        try {
            ...
            const daten: WiderspruchDaten = {
                caseCode, typ, bescheidDatum, versicherterName, pflegekasse, versicherungsnummer, strasse, plz, ort, begruendung
            };
            ...
            toast.error('Die Lizenzprüfung ist fehlgeschlagen. Bitte prüfen Sie Ihre Verbindung.', { id: verificationToast });
        } finally {
        ...
        <main className="min-h-screen bg-slate-900 py-12 px-4 text-white font-sans">
            <div className="container mx-auto max-w-3xl space-y-8">
                ...
                <div className="text-center mb-6">
                    ...
                    <p className="text-sm text-gray-400 mt-1">Gekoppelt an aktive Fallnummer: <span className="text-white font-mono font-bold">{caseCode || 'Keine (Kaltstart)'}</span></p>
                </div>

                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="grid w-full grid-cols-2 bg-white/5 border border-white/10 rounded-xl p-1 mb-6 text-white">
                        <TabsTrigger value="neu" className="data-[state=active]:bg-amber-500 data-[state=active]:text-slate-950 font-bold rounded-lg py-2 text-sm transition-all">Neues Anschreiben</TabsTrigger>
                        <TabsTrigger value="gespeichert" className="data-[state=active]:bg-amber-500 data-[state=active]:text-slate-950 font-bold rounded-lg py-2 text-sm transition-all">Gespeicherte Entwürfe ({gespeicherte.length})</TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent value="neu" className="space-y-6">
                        <Card className="bg-white/5 border-white/10 text-white shadow-xl">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(v: string) => {
                                            ...
                                            if (bescheidDatum && isDatumGueltig) {
                                                ...
                                                setFrist(zielFrist);
                                                const daten: WiderspruchDaten = {
                                                    caseCode, typ: neuerTyp, bescheidDatum, versicherterName, pflegekasse, versicherungsnummer, strasse, plz, ort, begruendung
                                                };
                                                setBriefText(generiereWiderspruchBrief(daten, zielFrist));
                                            }
                                        }}
                                    >
                                        ...
                                        <SelectContent className="bg-slate-950 border-white/10 text-white">
                                            ...
                                            <SelectItem value="mdk-gutachten">MDK-Gutachten Anforderung (§ 78 SGB X)</SelectItem>
                                ...
                                <div className="grid md:grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <Label htmlFor="name" className="text-xs text-gray-300 font-medium">Name des Versicherten *</Label>
                                        <Input id="name" value={versicherterName} onChange={(e) => setVersicherterName(e.target.value)} placeholder="z.B. Max Mustermann" className="bg-slate-950/50 border-white/10 h-11 text-white" />
                                    </div>
                                    <div className="space-y-1.5">
                                        <Label htmlFor="pflegekasse" className="text-xs text-gray-300 font-medium">Pflegekasse *</Label>
                                        <Input id="pflegekasse" value={pflegekasse} onChange={(e) => setPflegekasse(e.target.value)} placeholder="z.B. BARMER Pflegekasse" className="bg-slate-950/50 border-white/10 h-11 text-white" />
                                    </div>
                                </div>

                                <div className="space-y-1.5">
                                    <Label htmlFor="strasse" className="text-xs text-gray-300 font-medium">Straße und Hausnummer *</Label>
                                    <Input id="strasse" value={strasse} onChange={(e) => setStrasse(e.target.value)} placeholder="Musterstraße 12" className="bg-slate-950/50 border-white/10 h-11 text-white" />
                                </div>

                                <div className="grid grid-cols-3 gap-4">
                                    <div className="space-y-1.5 col-span-1">
                                        <Label htmlFor="plz" className="text-xs text-gray-300 font-medium">PLZ *</Label>
                                        <Input id="plz" value={plz} onChange={(e) => setPlz(e.target.value)} placeholder="12345" maxLength={5} className="bg-slate-950/50 border-white/10 h-11 text-white" />
                                    </div>
                                    <div className="space-y-1.5 col-span-2">
                                        <Label htmlFor="ort" className="text-xs text-gray-300 font-medium">Ort *</Label>
                                        <Input id="ort" value={ort} onChange={(e) => setOrt(e.target.value)} placeholder="Musterstadt" className="bg-slate-950/50 border-white/10 h-11 text-white" />
                                    </div>
                                </div>

                                <div className="space-y-1.5">
                                    <Label htmlFor="versicherungsnummer" className="text-xs text-gray-300 font-medium">Versicherungsnummer</Label>
                                    <Input id="versicherungsnummer" value={versicherungsnummer} onChange={(e) => setVersicherungsnummer(e.target.value)} placeholder="z.B. KV-Nummer eintragen" className="bg-slate-950/50 border-white/10 h-11 text-white" />
                                </div>

                                <div className="space-y-1.5">
                                    <Label htmlFor="begruendung" className="text-xs text-gray-300 font-medium">Begründung (Leer lassen für fristwahrenden Vorab-Widerspruch)</Label>
                                    <Textarea id="begruendung" value={begruendung} onChange={(e) => setBegruendung(e.target.value)} placeholder="Wird zur Fristwahrung unbegründet eingereicht..." rows={3} className="bg-slate-950/50 border-white/10 text-white resize-none" />
                                </div>
                        ...
                        {showErgebnis && frist && (
                            <>
                                <Card className={`bg-white/5 border-l-4 text-white shadow-md ${
                                    frist.ampelStatus === 'gruen' ? 'border-l-emerald-500' :
                                        frist.ampelStatus === 'gelb' ? 'border-l-amber-500' : 'border-l-rose-500'
                                }`}>
                                    <CardHeader>
                                ...
                                <Card className="bg-white/5 border-white/10 text-white shadow-xl">
                                    ...
                                    <CardContent>
                                        <div id="widerspruch-preview-zone" className="bg-slate-950/60 border border-white/10 rounded-xl p-4 font-mono text-xs text-gray-200 geometry-letter leading-relaxed whitespace-pre-wrap">
                                            {briefText}
                                        </div>
                                    </CardContent>
                                    <CardFooter className="flex flex-col sm:flex-row gap-3 pt-0">
                                        <Button onClick={() => setIsPreviewOpen(true)} className="flex-1 h-12 bg-[#20b2aa] hover:bg-[#3ddbd0] text-white font-bold rounded-xl shadow-md">
                                            <Eye className="mr-2 w-4 h-4" /> Gutachten-Vorschau öffnen
                                        </Button>
                                        <Button onClick={handleSpeichern} variant="outline" className="flex-1 h-12 border-white/10 text-white hover:bg-white/5 rounded-xl">
                                ...
                                <Card key={w.id} className="bg-white/5 border-white/10 text-white">
                                    <CardHeader className="pb-3 flex flex-row items-start justify-between gap-4">
                                        <div>
                                            ...
                                            <CardDescription className="text-xs text-gray-400">{w.pflegekasse} • Akte: {w.caseCode || 'Lokal'}</CardDescription>
                                        </div>
                                        <Button variant="ghost" size="sm" onClick={() => handleLoeschen(w.id)} className="hover:bg-rose-500/10 text-gray-400 hover:text-rose-400">
                ...
                <PdfPreviewModal
                    ...
                    onConfirmPrint={async () => { setIsPreviewOpen(false); await downloadPdf(); }}
                    briefText={briefText}
