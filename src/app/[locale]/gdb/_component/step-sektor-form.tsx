...
export function StepSektorForm({
                                   sektoren,
                                   werte,
                                   onChange,
                                   onBack,
    ...
    return (
        <Card className="bg-white/5 border-white/10 text-white shadow-xl">
            <CardHeader>
                <CardTitle>
                    Medizinische Funktionsbeeinträchtigungen
                </CardTitle>

                <CardDescription>
                    Wählen Sie für jeden Bereich die passende Stufe.
                </CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-6">
                {sektoren.map((sektor) => (
                    ...
                    >
                        ...
                        <select
                            ...
                            onChange={(e) =>
                                onChange(sektor.id, Number(e.target.value))
                            }
                        >
                            {sektor.werte.map((option) => (
                                <option key={option.label} value={option.wert}>
                                    {option.wert > 0
                                        ? `[GdB ${option.wert}] ${option.label}`
                                        : option.label}
                                </option>
                            ))}
                        </select>
                ...
                <div className="flex gap-3">
                    ...
                    <Button
                        onClick={onCalculate}
                        className="bg-[#20b2aa] text-slate-950"
                    >
