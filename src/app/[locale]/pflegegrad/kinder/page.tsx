      ...
      questions: [
        {
          ...
          options: [
            { value: 0, label: 'Altersgerecht selbstständig', simpleLabel: '😊 Altersgerecht - keine Hilfe nötig' },
            { value: 1, label: 'Leichte Verzögerung', simpleLabel: '😐 Manchmal Unterstützung oder Halten nötig' },
            { value: 2, label: 'Deutliche Einschränkung', simpleLabel: '😕 Häufiges Tragen/Hilfe erforderlich' },
            { value: 3, label: 'Vollständig unselbstständig', simpleLabel: '😟 Kann sich nicht allein fortbewegen' }
          ]
        }
      ]
    },
      ...
      questions: [
        {
          ...
          options: [
            ...
            { value: 1, label: 'Muss oft abgelenkt/erinnert werden', simpleLabel: '😐 Erfordert erhöhte Aufmerksamkeit' },
            { value: 2, label: 'Gefahrenbewusstsein fehlt stark', simpleLabel: '😕 Ständige Überwachung nötig' }
          ]
        }
      ]
    },
      ...
      questions: [
        {
          ...
          options: [
            { value: 0, label: 'Altersentsprechend', simpleLabel: '😊 Ohne medizinische Besonderheiten' },
            { value: 2, label: 'Erhöhter Zeitaufwand beim Füttern', simpleLabel: '😐 Essen dauert sehr lange / Hilfsmittel' },
            { value: 3, label: 'Sondenernährung / Schwere Störung', simpleLabel: '😟 Aufwendige Unterstützung bei jeder Mahlzeit' }
          ]
        }
      ]
    }
  ...
  if (ageGroup === 'baby') {
    return baseCategories.map(cat => ({
      ...
      questions: cat.questions.map(q => ({
        ...q,
        options: q.options.map(o => ({ ...o, label: o.label.replace('selbstständig', 'entwicklungskonform') }))
      }))
    }));
  }
...
const calculateChildCareLevel = (points: number, age: number): AssessmentResult => {
  ...
  if (isBaby) {
    ...
    if (points >= 90) return { level: 5, points, maxPoints: 100, description: 'Pflegegrad 5 (Schwerstpflegebedürftig mit besonderen Anforderungen)' };
    if (points >= 70) return { level: 5, points, maxPoints: 100, description: 'Pflegegrad 5 (Aufgrund gesetzlicher Baby-Höherstufung aus PG 4)' };
    if (points >= 47.5) return { level: 4, points, maxPoints: 100, description: 'Pflegegrad 4 (Aufgrund gesetzlicher Baby-Höherstufung aus PG 3)' };
    if (points >= 27) return { level: 3, points, maxPoints: 100, description: 'Pflegegrad 3 (Aufgrund gesetzlicher Baby-Höherstufung aus PG 2)' };
    if (points >= 12.5) return { level: 2, points, maxPoints: 100, description: 'Pflegegrad 2 (Einstiegsstufe für Babys unter 18 Monaten mit Einschränkungen)' };
    return { level: 0, points, maxPoints: 100, description: 'Kein Pflegegrad nachweisbar. Entwicklungsstand engmaschig dokumentieren.' };
  }

  // Normale Kinder-Einstufung (Ab 18 Monaten bis 18 Jahre)
  if (points >= 90) return { level: 5, points, maxPoints: 100, description: 'Pflegegrad 5 - Schwerste Beeinträchtigungen der Selbstständigkeit.' };
  if (points >= 70) return { level: 4, points, maxPoints: 100, description: 'Pflegegrad 4 - Schwerste Beeinträchtigungen.' };
  if (points >= 47.5) return { level: 3, points, maxPoints: 100, description: 'Pflegegrad 3 - Schwere Beeinträchtigungen.' };
  if (points >= 27) return { level: 2, points, maxPoints: 100, description: 'Pflegegrad 2 - Erhebliche Beeinträchtigungen.' };
  if (points >= 12.5) return { level: 1, points, maxPoints: 100, description: 'Pflegegrad 1 - Geringe Beeinträchtigungen.' };
...
export default function KinderModusPage(props: PageProps) {
  ...
  const handleNext = () => {
    if (currentCategory < categories.length - 1) {
      ...
      const berechneteSystemPunkte = maximalMöglicheRohpunkte > 0
        ...
        <div className="min-h-screen bg-slate-900 text-white p-4 flex flex-col justify-center items-center">
          <div className="max-w-xl mx-auto text-center space-y-6">
            ...
            <p className="text-gray-400 text-lg leading-relaxed">
              Spezial-Assessment zur Feststellung von Pflegebedürftigkeit bei Säuglingen, Kleinkindern und Jugendlichen nach dem SGB XI.
            </p>
            ...
            <div className="grid gap-3 text-left">
              <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl flex gap-3">
                ...
                <p className="text-sm text-gray-300"><strong>Altersgruppen-Vergleich:</strong> Abzug des natürlichen, altersbedingten Pflegebedarfs gesunder Kinder.</p>
              </div>
              <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl flex gap-3">
                <Sparkles className="w-5 h-5 text-pink-400 mt-0.5 flex-shrink-0" />
                <p className="text-sm text-gray-300"><strong>Baby-Sonderschutz:</strong> Berücksichtigung der pauschalen Höhergruppierung für Kinder unter 18 Monaten.</p>
              </div>
            </div>
        ...
        <div className="min-h-screen bg-slate-900 text-white p-4 flex flex-col justify-center items-center">
          <Card className="w-full max-w-md bg-white/5 border-white/10 text-white shadow-2xl">
            <CardHeader>
              ...
              <CardDescription className="text-gray-400">Ermöglicht das Laden der altersgerechten Vergleichsmatrizen.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {childInfo.age < 1.5 && (
                  <div className="p-3 bg-pink-500/10 border border-pink-500/20 rounded-xl flex gap-2 text-xs text-pink-400">
                    ...
                    <span><strong>Sonderregelung aktiv:</strong> Unter 18 Monaten greift der pauschale Ein-Stufen-Aufschlag. Pflegegrad 1 ist gesetzlich ausgeschlossen (Direkteinstieg in PG 2).</span>
                  </div>
              )}
            </CardContent>
            <CardFooter className="flex gap-3 border-t border-white/5 pt-4">
              <Button variant="outline" onClick={() => setStep('intro')} className="flex-1 border-white/10 text-white hover:bg-white/5">Zurück</Button>
              <Button onClick={() => setStep('assessment')} className="flex-1 bg-pink-600 hover:bg-pink-500 text-white font-semibold">Fragen laden</Button>
            </CardFooter>
          </Card>
        </div>
    );
            ...
            <div className="md:col-span-2 space-y-6">
              <div className="space-y-2">
                <div className="flex justify-between text-xs text-gray-400 font-mono">
                  <span>Bereich {currentCategory + 1} von {categories.length}</span>
              ...
              <Card className="bg-white/5 border-white/10 text-white shadow-xl">
                <CardHeader className="border-b border-white/5 pb-4">
                  <div className="flex items-center gap-3">
                    <div className={`p-3 rounded-xl bg-gradient-to-r ${currentCat.color} text-white shadow-lg`}>
                      {currentCat.icon}
                    </div>
                    <div>
                      <CardTitle className="text-xl">{currentCat.name}</CardTitle>
                      <CardDescription className="text-gray-400">Vergleichsmatrix für Altersgruppe: {childInfo.ageGroup.toUpperCase()}</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="p-6 space-y-6">
                  {currentQuestions.map((q) => (
                      <div key={q.id} className="space-y-3">
                        ...
                        <p className="text-xs text-gray-400 leading-relaxed font-mono bg-white/[0.02] p-2 rounded border border-white/5">{q.text}</p>
                        ...
                        >
                          {q.options.map((opt) => {
                            ...
                            return (
                                ...
                                >
                                  <RadioGroupItem value={opt.value.toString()} id={optionId} className="border-white/30 text-pink-500 focus:border-pink-500" />
                                  <label htmlFor={optionId} className="text-sm font-medium text-gray-200 cursor-pointer flex-grow select-none py-1">{opt.simpleLabel}</label>
                                </div>
                            );
                          })}
                        </RadioGroup>
                ...
                <CardFooter className="border-t border-white/5 pt-4 flex justify-between">
                  <Button variant="outline" onClick={handleBack} className="border-white/10 text-white hover:bg-white/5">Zurück</Button>
                  <Button
                      onClick={handleNext}
                      disabled={currentQuestions.some(q => answers[q.id] === undefined)}
                      className="bg-pink-600 hover:bg-pink-500 text-white font-bold"
                  >
                    {currentCategory < categories.length - 1 ? 'Nächster Bereich' : 'Analyse auswerten'}
            ...
            {!isUnlocked ? (
                ...
                <Card className="bg-slate-950 border-2 border-purple-500/30 text-white shadow-2xl overflow-hidden">
                  <div className="p-8 text-center space-y-4 bg-gradient-to-b from-purple-500/10 to-transparent">
                    ...
                    <h2 className="text-2xl font-extrabold tracking-tight">Rechtssicheres MD-Gutachten gesperrt</h2>
                    <p className="text-gray-400 text-sm max-w-sm mx-auto leading-relaxed">
                      Die Rohdaten wurden erfasst. Schalten Sie jetzt die professionelle SGB-XI Auswertungsmatrix und das fertige Antrags-PDF für Ihre Pflegekasse frei.
                    </p>
                  </div>
                  ...
                  <CardFooter className="p-6 border-t border-white/5 bg-white/[0.01] flex flex-col gap-3">
                    ...
                    <button onClick={() => setStep('assessment')} className="text-xs text-gray-500 hover:text-gray-400 font-mono underline">Eingaben korrigieren</button>
                  </CardFooter>
                </Card>
            ) : (
                ...
                <Card className="bg-white/5 border-white/10 text-white shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
                  <div className="p-8 bg-gradient-to-r from-pink-500/10 to-purple-500/10 border-b border-white/5 text-center space-y-2">
                    <span className="text-xs font-mono tracking-widest uppercase text-pink-400 bg-pink-500/10 px-3 py-1 rounded-full border border-pink-500/20">Analyse Entschlüsselt</span>
                    <h2 className="text-3xl font-extrabold">{result.level === 0 ? "Kein Pflegegrad" : `Voraussichtlich: Pflegegrad ${result.level}`}</h2>
                    <p className="text-sm text-gray-400 font-mono">Systempunkte: {result.points} / {result.maxPoints}</p>
                  </div>
                  <CardContent className="p-6 space-y-4">
                    ...
                    {childInfo.age < 1.5 && result.level >= 2 && (
                        <div className="p-4 bg-purple-500/10 border border-purple-500/20 rounded-xl flex gap-3 text-xs text-purple-400 leading-relaxed">
                          ...
                          <p><strong>Baby-Schutz angewendet:</strong> Da das Kind unter 18 Monaten alt ist, wurde die gesetzliche Höhergruppierung um eine Stufe vollautomatisch in das Ergebnis eingerechnet.</p>
                        </div>
                    )}
                  </CardContent>
                  <CardFooter className="border-t border-white/5 p-4 bg-white/[0.01]">
                    <Button onClick={() => setStep('intro')} className="w-full bg-white/5 border border-white/10 text-white hover:bg-white/10 h-12">Assessment schließen & neu starten</Button>
                  </CardFooter>
                </Card>
            )}
