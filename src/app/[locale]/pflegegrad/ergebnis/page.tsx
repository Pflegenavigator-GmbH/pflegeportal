...
export default function ErgebnisPage(props: PageProps) {
  ...
  const { downloadPdf, loadingPdf, showPaywall, setShowPaywall } = usePdfDownload({
    ...
    fallbackHtml: ergebnis ? `<h2>Zusammenfassung</h2><p>Errechneter Pflegegrad: ${ergebnis.careLevel}</p>` : undefined
  });
  ...
  if (!ergebnis) {
    return (
        <div className="container mx-auto px-4 py-12 text-center text-white bg-slate-900 min-h-screen flex flex-col justify-center items-center">
          ...
          <p className="text-gray-400 max-w-sm">Die SGB XI Schwellenwerte werden mit Ihren Angaben abgeglichen.</p>
        </div>
    );
  }

  const ampelKonfig: Record<EinstufungAmpel, { bg: string; text: string; border: string; label: string }> = {
    gruen: { bg: "bg-emerald-500/10", text: "text-emerald-400", border: "border-emerald-500/30", label: "Sicher über der gesetzlichen Schwelle" },
    gelb: { bg: "bg-amber-500/10", text: "text-amber-400", border: "border-amber-500/30", label: "Knapp über der gesetzlichen Schwelle (Grenzfall)" },
    rot: { bg: "bg-rose-500/10", text: "text-rose-400", border: "border-rose-500/30", label: "Unterhalb der gesetzlichen Mindest-Schwelle" },
  };
        ...
        <div id="nba-analysis-content" className="container mx-auto max-w-3xl space-y-8">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">Ermittelt nach den Begutachtungs-Richtlinien</p>
            </div>
            {hasMounted && caseCode && (
          ...
          <Card className={`bg-white/5 border-2 ${aktuelleAmpel.border} text-white shadow-2xl`}>
            <div className="p-8 sm:p-10 flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="space-y-3 text-center sm:text-left">
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold uppercase ${aktuelleAmpel.bg} ${aktuelleAmpel.text}`}>
                  {aktuelleAmpel.label}
                </span>
                <h2 className="text-4xl sm:text-5xl font-extrabold tracking-tight">
                  {ergebnis.careLevel === 0 ? "Kein Pflegegrad" : `Pflegegrad ${ergebnis.careLevel}`}
                </h2>
                <p className="text-sm text-gray-400 max-w-md">Punktwert: {ergebnis.totalScore.toFixed(1)} von 100.</p>
              </div>
              <div className={`w-28 h-28 rounded-2xl ${aktuelleAmpel.bg} border ${aktuelleAmpel.border} flex items-center justify-center flex-shrink-0`}>
          ...
          {ergebnis.careLevel > 0 && (
              <Card className="bg-white/5 border-white/10 text-white shadow-xl">
                ...
                <CardContent className="p-6">
                  ...
                  <Button variant="ghost" onClick={() => router.push(`/${locale}/kombileistungen`)} className="w-full h-12 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/20 rounded-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Button variant="outline" onClick={downloadPdf} disabled={loadingPdf} className="h-14 border-white/10 text-white hover:bg-white/5 shadow-md">
              <Download className="w-4 h-4 mr-2" /> PDF
            </Button>
            <Button variant="outline" onClick={shareErgebnis} className="h-14 border-white/10 text-white hover:bg-white/5 shadow-md">
              <Share2 className="w-4 h-4 mr-2" /> Teilen
            </Button>
            <Button onClick={() => router.push(`/${locale}/briefe`)} className="h-14 bg-[#20b2aa] hover:bg-[#3ddbd0] text-white font-bold shadow-xl">
          ...
          <Card className="bg-gradient-to-r from-white/5 to-transparent border-white/10 text-white p-5 rounded-xl shadow-xl">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-center sm:text-left">
                ...
                <p className="text-gray-400 text-xs">Erhalten Sie Steuerfreibeträge und Zusatzurlaub.</p>
              </div>
              <Button onClick={handleGdbNavigation} disabled={isVerifyingGdb} className="bg-[#20b2aa] hover:bg-[#3ddbd0] text-slate-950 font-bold text-xs h-10 rounded-xl">
