...
import {
    Phone, MapPin, Search, Star, Users, GraduationCap, Heart, AlertCircle,
    LifeBuoy, MessageCircle, ExternalLink, ChevronDown, ChevronUp, Euro, Clock, ArrowLeft
} from "lucide-react";
...
const kursAnbieter = [
    { id: 1, name: "Deutsches Rotes Kreuz", ort: "Deutschlandweit", kosten: "0 € (Kassenleistung § 45)", dauer: "Kompaktkurse / Online", telefon: "0800-123456" },
    { id: 2, name: "Johanniter Unfallhilfe", ort: "Deutschlandweit", kosten: "0 € (Kassenleistung § 45)", dauer: "Abendkurse / Präsenz", telefon: "0800-654321" }
];

const notfallKontakte = [
    { nummer: "112", name: "Rettungsdienst & Notarzt", beschreibung: "Bei akuter Lebensgefahr" },
    { nummer: "116 117", name: "Ärztlicher Bereitschaftsdienst", beschreibung: "Außerhalb der Praxisöffnungszeiten" },
    { nummer: "0800 111 0 111", name: "Telefonseelsorge / Krisenhilfe", beschreibung: "Kostenfreie psychosoziale Beratung" },
];
        ...
        <div className="min-h-screen bg-slate-900 text-white font-sans">
            <header className="bg-slate-950/40 border-b border-white/10 py-6">
                <div className="max-w-6xl mx-auto px-4">
                    ...
                    <p className="text-[#20b2aa] text-sm mt-1">Regionale Dienstleister, kostenlose Pflegekurse und Notfall-Schnittstellen</p>
                </div>
            </header>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <Card className="bg-white/5 border-white/10 text-white shadow-xl">
                            <CardHeader className="border-b border-white/5 pb-4">
                                ...
                                <CardDescription className="text-gray-400 text-xs">Gleichen Sie freie Kapazitäten über die Supabase-Cluster ab</CardDescription>
                            </CardHeader>
                            <CardContent className="pt-4">
                                ...
                                <div className="space-y-2">
                                    {dienste.map((dienst) => (
                                        <div key={dienst.id} className="border border-white/5 bg-slate-950/20 rounded-xl p-4 flex justify-between items-center hover:bg-white/5 transition-all">
                                            <div>
                                                <h3 className="font-semibold text-white text-sm">{dienst.name}</h3>
                                                <p className="text-xs text-gray-500 mt-0.5">{dienst.strasse || 'Hauptstraße'}</p>
                                                <div className="flex items-center gap-4 mt-1.5 text-xs text-gray-400">
                                                    <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> {dienst.plz} {dienst.stadt}</span>
                                                    {dienst.telefon && (
                                                        <span className="flex items-center gap-1"><Phone className="w-3.5 h-3.5" /> {dienst.telefon}</span>
                                                    )}
                                                </div>
                                            </div>
                                    ...
                                    {!loading && suchOrt.length >= 2 && dienste.length === 0 && (
                                        <p className="text-xs text-gray-500 text-center py-4">Keine registrierten Dienste für diese Eingabe gefunden.</p>
                                    )}
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="bg-white/5 border-white/10 text-white shadow-xl">
                            <CardHeader className="border-b border-white/5 pb-4">
                                <CardTitle className="text-lg font-bold flex items-center gap-2">
                                    <GraduationCap className="w-5 h-5 text-[#20b2aa]" /> Schulungen für Angehörige (§ 45 SGB XI)
                                </CardTitle>
                            </CardHeader>
                            <CardContent className="pt-4 space-y-4">
                                <div className="bg-[#20b2aa]/10 border border-20b2aa/20 p-4 rounded-xl text-xs text-gray-300">
                                    <strong>Gesetzes-Anspruch:</strong> Pflegekassen bieten Angehörigen unentgeltliche Schulungskurse an, um die Pflege zu erleichtern.
                                </div>
                                <div className="space-y-2">
                                    {kursAnbieter.map((anbieter) => (
                                        <div key={anbieter.id} className="border border-white/5 bg-slate-950/20 rounded-xl overflow-hidden">
                                            <button
                                            ...
                                            >
                                                ...
                                                {expandedKurs === anbieter.id ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
                                            </button>
                                            {expandedKurs === anbieter.id && (
                                                <div className="p-4 bg-slate-950/40 border-t border-white/5 text-xs space-y-2 text-gray-300">
                                                    <p className="flex items-center gap-2"><MapPin className="w-3.5 h-3.5 text-gray-500" /> {anbieter.ort}</p>
                                                    <p className="flex items-center gap-2"><Euro className="w-3.5 h-3.5 text-emerald-400" /> {anbieter.kosten}</p>
                                                    <p className="flex items-center gap-2"><Clock className="w-3.5 h-3.5 text-gray-500" /> {anbieter.dauer}</p>
                            ...
                            <CardContent className="pt-4">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    {notfallKontakte.map((kontakt, i) => (
                                        <a key={i} href={`tel:${kontakt.nummer.replace(/\s/g, "")}`} className="p-4 bg-slate-950/40 border border-white/5 rounded-xl hover:border-rose-500/30 text-center block transition-all group">
                                            <p className="text-xl font-extrabold text-rose-400 group-hover:scale-105 transition-transform">{kontakt.nummer}</p>
                                            <p className="font-bold text-xs text-white mt-1">{kontakt.name}</p>
                                            <p className="text-[10px] text-gray-400 mt-1 leading-snug">{kontakt.beschreibung}</p>
                                        </a>
                                    ))}
                                </div>
                            </CardContent>
                        ...
                        <div className="sticky top-20">
                            <Card className="bg-white/5 border-white/10 overflow-hidden shadow-2xl">
                                ...
                                <CardContent className="p-0 h-[420px] bg-slate-950/20">
                                    <Suspense fallback={<div className="h-full flex items-center justify-center text-xs text-gray-500 animate-pulse">Kompiliere Audio-Schnittstelle...</div>}>
                                        <AvatarChat
