...
export async function POST(request: Request) {
  try {
    ...
    if (!caseCode || !content) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });

    const supabase = await createServerSupabaseClient();
    const { data: caseData } = await supabase.from('cases').select('id').eq('case_code', caseCode).single();
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!caseCode || !entryKey) return NextResponse.json({ error: 'Missing params' }, { status: 400 });
    ...
    const { data: existing } = await supabase.from('answers').select('answers').eq('case_id', caseData!.id).eq('module_number', 5).single();

    const answers = existing?.answers || {};
    delete answers[entryKey]; // Eintrag entfernen

    // Speichern
    await supabase.from('answers').update({ answers }).eq('case_id', caseData!.id).eq('module_number', 5);
