...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (paket === 'beta_special') {
      ...
      if (aktuelleBetaNutzer >= 1000) {
        ...
        throw new ValidationError(
          'Das exklusive Kontingent für das Beta-Special (1.000 Plätze) ist restlos ausverkauft.'
        );
      }
    }
    ...
    if (productError || !productDb) {
      console.error(
        `❌ [Checkout DB-ERROR] Kein passendes Produkt in DB gefunden für Paket: ${paket}, Intervall: ${gesuchtesInterval}, isDev: ${isDev}`
      );
      throw new ValidationError(
    ...
    console.log(
      `✅ [Checkout SUCCESS] Gemapptes Produkt gefunden: "${productDb.name}" (ID: ${priceId})`
    );

    // Erstellung der Stripe-Checkout-Session
    console.log(
      `[Checkout STRIPE] Initialisiere Stripe Session (Modus: ${paket.includes('monthly') || paket.includes('yearly') ? 'subscription' : 'payment'})...`
    );
    ...
    if (updatePendingError) {
      console.error(
        `❌ [Checkout DB-ERROR] Konnte Status nicht auf pending setzen:`,
        updatePendingError
      );
    } else {
