...
export default async function proxy(request: NextRequest) {
  ...
  const previewProtectionEnabled =
  ...
  if (previewProtectionEnabled) {
    ...
    if (!tokenSecret) {
      console.error(
          '[preview-auth] PREVIEW_TOKEN_SECRET ist nicht konfiguriert.'
      );

      return new NextResponse(
          'Preview protection is not configured correctly.',
          {
            status: 500,
          }
      );
    ...
    const hasPreviewAccess = token
        ? await verifyPreviewToken(token, tokenSecret)
        : false;
  ...
  if (pathname.startsWith('/api/')) {
    try {
      ...
      console.error(
          '[middleware] API-Pipeline fehlgeschlagen, lasse Anfrage durch:',
          error
      );
