...
function decodePayload(value: string): PreviewTokenPayload | null {
  try {
 ...
 */
export async function verifyPreviewToken(token: string, secret: string): Promise<boolean> {
  try {
...
export const PREVIEW_SESSION_MAX_AGE = PREVIEW_SESSION_DURATION_SECONDS;
