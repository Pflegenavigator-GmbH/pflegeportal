...
export function calculatePflegegrad(scores: Partial<ModuleScores>): PflegegradErgebnis {
    ...
    const totalScore = Math.round((weightedScores[1] + weightedScores[23] + weightedScores[4] + weightedScores[5]) * 10) / 10;
    ...
    return {
        ...
        missingData,
        benefits: {
            monthlyAmount: careLevel === 2 ? 347 : careLevel === 3 ? 599 : careLevel === 4 ? 800 : careLevel === 5 ? 990 : 0,
            reliefBudget: careLevel >= 1 ? 131 : 0,
            additionalBenefits: careLevel >= 2 ? ['Pflegehilfsmittel: 42 €/Monat', 'Wohnraumanpassung: 4.180 €'] : []
        },
        recommendations: careLevel === 0 ? ['Wiederholung in 6 Monaten prüfen'] : ['Antrag auf Schwerbehindertenausweis prüfen']
    };
}