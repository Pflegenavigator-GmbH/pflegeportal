...
export class BrevoClient {
  ...
  private constructor() {
    ...
    logger.debug(
      ...
      'BrevoClient Singleton-Instanz wurde erfolgreich erstellt.'
    );
  }
  ...
  public async sendEmail(options: SendEmailOptions): Promise<void> {
    const sender = options.sender || {
      ...
      email: 'noreply@pflegenavigatoreu.com',
    };
    const payload = {
      sender,
      to: options.to,
    ...
    try {
      ...
      logger.error(
        { action: 'brevo_email_exception', err: error },
  ...
  public async sendSms(options: SendSmsOptions): Promise<void> {
    ...
    try {
      ...
      logger.error(
        { action: 'brevo_sms_exception', err: error },
        'Unerwarteter Fehler beim SMS-Versand'
      );
      throw error;
    }
  }
