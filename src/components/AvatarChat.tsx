...
export default function AvatarChat({
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        const errorMessage: Message = {
          ...
          role: 'assistant',
          content:
            'Entschuldigung, es gab einen Fehler bei der Übertragung. Bitte versuchen Sie es erneut.',
          timestamp: new Date(),
        };
    ...
    [messages, topic, voiceEnabled, speakText]
  );
  ...
  // 4. Den Ref immer mit der aktuellsten Version synchronisieren
  useEffect(() => {
    handleVoiceCommandRef.current = handleVoiceCommand;
  }, [handleVoiceCommand]);

  // 5. Initialisierung der Speech Recognition (Startet jetzt nur noch EINMAL!)
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (SpeechRecognition) {
        ...
        recognition.onresult = (event: ISpeechRecognitionEvent) => {
          ...
          // Wir nutzen den Ref, damit wir immer die aktuellste Funktion haben!
    ...
    <div className="flex flex-col h-full bg-slate-900 border border-white/10 text-white overflow-hidden">
      ...
      <div className="bg-slate-950/60 p-4 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            >
              <Bot
                className={`w-5 h-5 ${avatarAnimation === 'talking' ? 'text-[#20b2aa]' : 'text-white'}`}
              />
            </div>
      ...
      {showHints && (
        <div className="bg-amber-500/10 px-4 py-2 flex items-center gap-2 text-xs text-amber-300 border-b border-amber-500/20">
          ...
          <span className="bg-slate-950/40 px-1.5 py-0.5 rounded border border-white/10">
            Hilfe
          </span>
          <span className="bg-slate-950/40 px-1.5 py-0.5 rounded border border-white/10">
            Weiter
