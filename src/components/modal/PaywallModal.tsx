...
export function PaywallModal({ caseCode, isExpired, products, onCheckout, onClose, loading }: PaywallModalProps) {
    return (
                ...
                <Card className="bg-slate-900 border-blue-500/40 text-white shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <CardContent className="space-y-4">
                        <p className="text-sm text-gray-300 text-center">
                            Die Standard-Pflegegrad-Einschätzung ist und bleibt für Sie **kostenlos**.
                            Hier können Sie optionale Premium-Funktionen oder den PDF-Export freischalten.
                        </p>
                        <div className="grid gap-4 mt-4">
                            {products.map((product) => (
                                <div key={product.id} className="p-4 bg-white/5 border border-white/10 rounded-xl flex items-center justify-between">
                                    <div>
                                        <p className="font-semibold text-white">{product.name}</p>
                                        <p className="text-xs text-gray-400">Inklusive vollem Funktionsumfang</p>
                                    </div>
                                    <Button onClick={() => onCheckout(product.id)} disabled={loading} className="bg-blue-600 hover:bg-blue-500 text-white font-bold">
                    ...
                    <CardFooter>
                        <Button variant="ghost" onClick={onClose} className="w-full text-gray-400 hover:text-white">
